#!/bin/bash		
#
# data.sh - backup data partition to external hard disk
#
# Tobin Harding
shopt -s -o nounset
shopt -s -o noclobber
...
# Config
SRC='/home/tobin/data/'				# must have trailing slash
DST="/mnt/hd/backup/data"			# no trailing slash
...
MAX=20			        # number of incremental backups to keep
...
if [ ! -f "$EXCL" ]; then	# rsync exclude list
    printf "$SCRIPT: file $EXCL does not appear to exist - aborting\n" >&2
    exit 192
fi
...
# assert /dev/sda8 is mounted 
mount -l | grep /dev/sda8 > /dev/null # list mounts
if [ $? -ne 0 ]; then
    echo "/dev/sda8 does not appear to be mounted .. aborting"
    exit 1
fi
